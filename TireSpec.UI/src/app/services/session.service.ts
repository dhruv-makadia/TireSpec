...
export class SessionService {
  ...
  createSession(websiteId: string): Observable<CreateSessionResponse> {
    ...
    return this.http
      ...
      .pipe(tap((response) => this.storeToken(response.jwt, websiteId)));
  }

  getToken(guid: string): string | null {
    const match = new RegExp(new RegExp(`(?:^|; )${this.cookieName}_${guid}=([^;]*)`)).exec(
      document.cookie,
    );
  ...
  storeToken(jwt: string, guid: string): void {
    let cookieStr = `${this.cookieName}_${guid}=${encodeURIComponent(jwt)}; path=/; SameSite=Strict`;
